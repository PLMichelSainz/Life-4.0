...
export default function TransportExpenses() {
  ...
  const totales = dias.reduce(
    (acc, fecha) => {
      if (fecha < hoy) return acc // los días ya transcurridos no cuentan en el total
    ...
    <div>
      ...
      <div className="card">
        ...
        <div className="grid cols-2">
          <div className="stat">
            <div className="label">Falta por cubrir (redondeado)</div>
            <div className="value mono">{formatMXN(recarga.faltaRedondeada)}</div>
          </div>
          <div className="stat" style={{ borderColor: falta > 0 ? 'var(--accent)' : 'var(--border-soft)' }}>
            <div className="label">Monto a transferir (en centavos, incluye comisión)</div>
      ...
      <div className="card">
        ...
        {dias.map((fecha) => {
          ...
          // un día ya transcurrido se muestra en 0 y deja de contarse en el total
          const registro = bloqueado ? { normal: 0, transbordo: 0 } : transporte[fecha] || { normal: 0, transbordo: 0 }
